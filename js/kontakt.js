...
/**
 * Sendet das Kontaktformular ab an das PHP-Backend
 */
function submitContactForm() {
    ...
    // Debug: Zeige Formulardaten in Konsole
    ...
    // Formular an PHP-Backend senden
    fetch('kontakt-senden.php', {
        method: 'POST',
        body: formData
    })
    .then(response => {
    ...
    .then(text => {
        ...
        try {
            ...
            showErrorMessage('Server-Fehler: ' + text.substring(0, 200));
        ...
        if (data.success) {
            ...
            showErrorMessage(data.message || 'Ein Fehler ist aufgetreten. Bitte versuchen Sie es erneut.');
            resetSubmitButton(submitButton);
        }
    })
    .catch(error => {
        console.error('Fehler beim Senden:', error);
        showErrorMessage('Verbindungsfehler. Bitte versuchen Sie es später erneut oder kontaktieren Sie uns telefonisch.');
        resetSubmitButton(submitButton);
    });
}

/**
 * Setzt den Submit-Button auf den ursprünglichen Status zurück
 */
...
function showSuccessMessage(message) {
    ...
    successMessage.innerHTML = `
        ...
        <span>${message || 'Vielen Dank für Ihre Nachricht! Wir werden uns so schnell wie möglich bei Ihnen melden.'}</span>
    `;
